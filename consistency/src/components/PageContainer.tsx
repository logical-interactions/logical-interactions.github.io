...
export default class PageContainer extends React.Component<undefined, undefined> {
  render() {
    return (<div>
      <h1>Consistency of Aysnchronous Interactions</h1>
      <h2 style={{textAlign: "right", lineHeight: "70%"}}>Or, How to Time Travel Safely with R4</h2>
      <h2>Life of an Interaction</h2>
      <p>Not all interactions are created equal.  When we interact with an object, say, picking up a pen, there is immediate feedback, and immediate consequences.  The interaction with the pen was instsantenous.  Sometimes we can also interact with objects that are slow, for instance controlling the rudder of a boat.  Fortunately, the rules of physics is fixed and through practice, we learn how to work with them.  However, when we interact with with a computer interface, all bets are off---the widget could not respond to your clicks, the widget could move slightly after your initial instructions, the results might show up sometime after your initial mouseevents, there may be a spinner (but you don't really know what it's referring to.  The list goes on.</p>
      <p>One "solution" is to make things fast—faster rendering, faster network, faster data processing.  However we like to add features as well—larger data volume, fancier algorithms, larger screens, more business logic.  Point being, we are not likely to every do away with some form of slowness.  So we should take some time(!) to understand what’s going on.  Now we discuss a simple model that could make computer interactions a bit more like controlling a rudder, than throwing a dice.</p>
      ...
      <h2>The Model</h2>
      <p>For asynchrony to make sense, it's better to consider visualization updates as tasks.  </p>
      <Model
        width={500}
        height={500}
      />
      ...
      <p>While there are consistency models in databases, it doesn't directly translate to the UI (we <a href="http://people.eecs.berkeley.edu/~yifanwu/assets/devil.pdf">tried</a>!). </p>
      <h2>The Designs</h2>
      ...
      <p>R4 is for inte<b>r</b>act, <b>r</b>equest, <b>r</b>esponse, <b>r</b>ender.  The idea is to wrap the developers' functions into the model's four r's, and then apply higher level policies <i>declaratively</i>.  Some were already discussed before in the model section.</p>
